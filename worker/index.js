...
async function checkAssignment(id,request,env){const s=await requireSession(request,env,"student");if(s instanceof Response)return s;if(Number(request.headers.get("content-length")||0)>12000)return json({error:"too_large"},413);const body=await request.json(),submitted=Array.isArray(body.answers)?body.answers.map(x=>String(x).slice(0,200)):[];const set=await env.DB.prepare("SELECT id,question_count,salt,hashes_json,answer_keys_json FROM assignments WHERE id=? AND is_active=1").bind(id).first();if(!set)return json({error:"not_found"},404);if(body.revision&&body.revision!==await digest(set.salt))return json({error:"key_changed"},409);if(submitted.length!==set.question_count)return json({error:"invalid_answers"},400);const now=new Date().toISOString();await env.DB.prepare("INSERT OR IGNORE INTO assignment_progress (assignment_id,user_id,current_answers_json,results_json,attempt_count,updated_at) VALUES (?,?,?,?,?,?)").bind(id,s.userId,JSON.stringify(Array(set.question_count).fill("")),JSON.stringify(Array(set.question_count).fill(false)),0,now).run();const old=await env.DB.prepare("SELECT current_answers_json,results_json,attempt_count,started_at,completed_at FROM assignment_progress WHERE assignment_id=? AND user_id=?").bind(id,s.userId).first();if(!old)return json({error:"try_again"},409);if(body.attemptCount!==undefined&&body.attemptCount!==Number(old.attempt_count||0))return json({error:"try_again"},409);const previous=parse(old.current_answers_json,Array(set.question_count).fill("")),oldResults=await evaluateAnswers(previous,set),answers=submitted.map((a,i)=>oldResults[i]?previous[i]:a);if(answers.some((x,i)=>!oldResults[i]&&(!normalizeAnswer(x)||x.includes("□"))))return json({error:"missing_answer"},400);const results=await evaluateAnswers(answers,set);const attempt=Number(old.attempt_count||0)+1,complete=results.every(Boolean)?(old.completed_at||now):null;const saved=await env.DB.prepare("UPDATE assignment_progress SET current_answers_json=?,results_json=?,attempt_count=?,started_at=?,updated_at=?,completed_at=? WHERE assignment_id=? AND user_id=? AND attempt_count=? AND EXISTS (SELECT 1 FROM assignments WHERE id=? AND salt=?)").bind(JSON.stringify(answers),JSON.stringify(results),attempt,old.started_at||now,now,complete,id,s.userId,Number(old.attempt_count||0),id,set.salt).run();if(!saved.meta?.changes)return json({error:"try_again"},409);if(Number(old.attempt_count||0)>0){const changes=[];for(let i=0;i<answers.length;i++){if(!oldResults[i]&&normalizeAnswer(previous[i])!==normalizeAnswer(answers[i]))changes.push(env.DB.prepare("INSERT INTO answer_changes (assignment_id,user_id,question_index,attempt_number,previous_answer,new_answer,is_correct,changed_at) VALUES (?,?,?,?,?,?,?,?)").bind(id,s.userId,i,attempt,previous[i],answers[i],results[i]?1:0,now))}if(changes.length)await env.DB.batch(changes)}return json({revision:await digest(set.salt),currentAnswers:answers.map((a,i)=>results[i]?"":a),results,attemptCount:attempt,completedAt:complete})}
...
async function evaluateAnswers(answers,set){const hashes=parse(set.hashes_json,[]),keys=parse(set.answer_keys_json,null);return Promise.all(Array.from({length:set.question_count},async(_,i)=>{if(!normalizeAnswer(answers[i]))return false;if(keys?.[i]){const norm=keys[i].type==='choice'?normalizeAnswer:mathNotation;return answerVariants(keys[i].answer).some(v=>norm(v)===norm(answers[i]))}return !!hashes[i]?.includes(await answerHash(set.salt,answers[i]))}))}
...
function validQuestions(body){const qs=Array.isArray(body.questions)?body.questions:[];if(qs.length<1||qs.length>40||qs.some(q=>!q||!String(q.answer||"").trim()||String(q.answer).includes("□")))return null;const questions=qs.map(q=>({type:q.type==="choice"?"choice":"short",answer:String(q.answer).slice(0,300).trim()}));for(const q of questions){if(q.type==="choice"){const options=q.answer.normalize("NFKC").split("|").map(x=>x.trim());if(options.some(x=>!["1","2","3","4","5"].includes(x)))return null;q.answer=options.join("|")}}return questions}
