...
async function saveQuestion(request,env){
 ...
 const body=parse(raw,null);if(!body||typeof body.assignmentId!=="string"||!Number.isInteger(body.question))return json({error:"invalid_question"},400);
 ...
 if(old&&old.status!=="resolved")return json({ok:true,alreadyExists:true});
 const now=new Date().toISOString(),item={studentName:auth.name,assignmentTitle:row.title,assignmentId:row.id,question:body.question,createdAt:old?.createdAt||now,updatedAt:now,status:"open",requestCount:(old?.requestCount|| (old?1:0))+1,history:[...(old?.history||[]),{action:old?"reopened":"asked",at:now}]};
