...
async function getAssignment(id,request,env){const s=await requireSession(request,env,"student");if(s instanceof Response)return s;const row=await env.DB.prepare("SELECT a.id,a.title,a.question_count,a.types_json,a.salt,a.hashes_json,a.answer_keys_json,a.worksheet_key,a.worksheet_name,a.worksheet_type,p.current_answers_json,p.results_json,p.attempt_count,p.completed_at FROM assignments a LEFT JOIN assignment_progress p ON p.assignment_id=a.id AND p.user_id=? WHERE a.id=? AND a.is_active=1").bind(s.userId,id).first();if(!row)return json({error:"not_found"},404);const answers=parse(row.current_answers_json,Array(row.question_count).fill("")),results=await evaluateAnswers(answers,row);if(Number(row.attempt_count||0)>0){const complete=results.every(Boolean)?row.completed_at||new Date().toISOString():null;const sync=await env.DB.prepare("UPDATE assignment_progress SET results_json=?,completed_at=? WHERE assignment_id=? AND user_id=? AND attempt_count=? AND current_answers_json=? AND EXISTS (SELECT 1 FROM assignments WHERE id=? AND salt=?)").bind(JSON.stringify(results),complete,id,s.userId,row.attempt_count,row.current_answers_json,id,row.salt).run();if(!sync.meta?.changes)return json({error:"try_again"},409);row.completed_at=complete;}const draftData=await readDraft(id,s.userId,row,Number(row.attempt_count||0),env);if(draftData.draft)draftData.draft.answers=draftData.draft.answers.map((a,i)=>results[i]?"":a);return json({...draftData,revision:await digest(row.salt),id:row.id,title:row.title,questionCount:row.question_count,types:parse(row.types_json,[]),hasWorksheet:!!row.worksheet_key,worksheetName:row.worksheet_name||null,worksheetType:row.worksheet_type||null,worksheetUrl:row.worksheet_key?"/worksheet/"+encodeURIComponent(row.id):null,currentAnswers:answers.map((a,i)=>results[i]?"":a),results,attemptCount:Number(row.attempt_count||0),completedAt:row.completed_at||null})}
async function checkAssignment(id,request,env){const s=await requireSession(request,env,"student");if(s instanceof Response)return s;if(Number(request.headers.get("content-length")||0)>12000)return json({error:"too_large"},413);const body=await request.json(),submitted=Array.isArray(body.answers)?body.answers.map(x=>String(x).slice(0,200)):[];const set=await env.DB.prepare("SELECT id,question_count,salt,hashes_json,answer_keys_json FROM assignments WHERE id=? AND is_active=1").bind(id).first();if(!set)return json({error:"not_found"},404);if(body.revision&&body.revision!==await digest(set.salt))return json({error:"key_changed"},409);if(submitted.length!==set.question_count)return json({error:"invalid_answers"},400);const now=new Date().toISOString();await env.DB.prepare("INSERT OR IGNORE INTO assignment_progress (assignment_id,user_id,current_answers_json,results_json,attempt_count,updated_at) VALUES (?,?,?,?,?,?)").bind(id,s.userId,JSON.stringify(Array(set.question_count).fill("")),JSON.stringify(Array(set.question_count).fill(false)),0,now).run();const old=await env.DB.prepare("SELECT current_answers_json,results_json,attempt_count,started_at,completed_at FROM assignment_progress WHERE assignment_id=? AND user_id=?").bind(id,s.userId).first();if(!old)return json({error:"try_again"},409);if(body.attemptCount!==undefined&&body.attemptCount!==Number(old.attempt_count||0))return json({error:"try_again"},409);const previous=parse(old.current_answers_json,Array(set.question_count).fill("")),oldResults=await evaluateAnswers(previous,set),answers=submitted.map((a,i)=>oldResults[i]?previous[i]:a);if(answers.some((x,i)=>!oldResults[i]&&separatedNumbers(x)))return json({error:"comma_required"},400);if(answers.some((x,i)=>!oldResults[i]&&(!normalizeAnswer(x)||x.includes("□"))))return json({error:"missing_answer"},400);const results=await evaluateAnswers(answers,set);const attempt=Number(old.attempt_count||0)+1,complete=results.every(Boolean)?(old.completed_at||now):null;const saved=await env.DB.prepare("UPDATE assignment_progress SET current_answers_json=?,results_json=?,attempt_count=?,started_at=?,updated_at=?,completed_at=? WHERE assignment_id=? AND user_id=? AND attempt_count=? AND EXISTS (SELECT 1 FROM assignments WHERE id=? AND salt=?)").bind(JSON.stringify(answers),JSON.stringify(results),attempt,old.started_at||now,now,complete,id,s.userId,Number(old.attempt_count||0),id,set.salt).run();if(!saved.meta?.changes)return json({error:"try_again"},409);if(Number(old.attempt_count||0)>0){const changes=[];for(let i=0;i<answers.length;i++){if(!oldResults[i]&&normalizeAnswer(previous[i])!==normalizeAnswer(answers[i]))changes.push(env.DB.prepare("INSERT INTO answer_changes (assignment_id,user_id,question_index,attempt_number,previous_answer,new_answer,is_correct,changed_at) VALUES (?,?,?,?,?,?,?,?)").bind(id,s.userId,i,attempt,previous[i],answers[i],results[i]?1:0,now))}if(changes.length)await env.DB.batch(changes)}return json({revision:await digest(set.salt),currentAnswers:answers.map((a,i)=>results[i]?"":a),results,attemptCount:attempt,completedAt:complete})}
...
// Structural notation comparison, not symbolic solving or numeric approximation.
function answerVariants(value){const text=String(value).trim();return /^\|[^|]+\|$/.test(text)?['abs('+text.slice(1,-1)+')']:text.split('|').map(v=>v.trim()).filter(Boolean).slice(0,8)}
// Exact arithmetic only: no floating point tolerance that could accept a near miss.
function exactNumber(tree){
 const limit=2048;
 function rat(n,d=1n){if(!d)throw Error();if(d<0n){n=-n;d=-d}let a=n<0n?-n:n,b=d;while(b){const r=a%b;a=b;b=r}n/=a;d/=a;if(n.toString(2).length>limit||d.toString(2).length>limit)throw Error();return [n,d]}
 function squareRoot(n){if(n<0n)throw Error();if(n<2n)return n;let x=n,y=(x+1n)/2n;while(y<x){x=y;y=(x+n/x)/2n}if(x*x!==n)throw Error();return x}
 function walk(t){const op=t[0];if(op==='number'){const [a,b='']=t[1].split('.');return rat(BigInt((a||'0')+b),10n**BigInt(b.length))}if(op==='neg'){const [n,d]=walk(t[1]);return [-n,d]}if(op==='sqrt'){const [n,d]=walk(t[1]);return rat(squareRoot(n),squareRoot(d))}if(op==='call'&&t[1]==='abs'&&t[2].length===1){const [n,d]=walk(t[2][0]);return [n<0n?-n:n,d]}if(!['+','-','*','/','^'].includes(op))throw Error();const [a,b]=walk(t[1]),[c,d]=walk(t[2]);if(op==='+')return rat(a*d+c*b,b*d);if(op==='-')return rat(a*d-c*b,b*d);if(op==='*')return rat(a*c,b*d);if(op==='/')return rat(a*d,b*c);if(d!==1n||c>32n||c< -32n||a===0n&&c<=0n)throw Error();return c<0n?rat(b**(-c),a**(-c)):rat(a**c,b**c)}
 try{const [n,d]=walk(tree);return 'number:'+n+'/'+d}catch{return null}
}
function mathNotation(value){
 const supers='⁰¹²³⁴⁵⁶⁷⁸⁹',fractions={'½':'(1)/(2)','⅓':'(1)/(3)','⅔':'(2)/(3)','¼':'(1)/(4)','¾':'(3)/(4)','⅕':'(1)/(5)','⅖':'(2)/(5)','⅗':'(3)/(5)','⅘':'(4)/(5)','⅙':'(1)/(6)','⅚':'(5)/(6)','⅛':'(1)/(8)','⅜':'(3)/(8)','⅝':'(5)/(8)','⅞':'(7)/(8)'};
 let source=String(value??'').trim();if(/^\|[^|]+\|$/.test(source))source='abs('+source.slice(1,-1)+')';
 source=source.replace(/[⁰¹²³⁴⁵⁶⁷⁸⁹]+/g,x=>'^('+[...x].map(c=>supers.indexOf(c)).join('')+')').normalize('NFC').replace(/[！-～]/g,c=>String.fromCharCode(c.charCodeAt(0)-0xfee0)).replace(/[½⅓⅔¼¾⅕⅖⅗⅘⅙⅚⅛⅜⅝⅞]/g,c=>fractions[c]).replace(/\s+/g,'').replace(/[−–]/g,'-').replace(/[×·]/g,'*').replace(/[÷⁄]/g,'/').replace(/<=/g,'≤').replace(/>=/g,'≥').replace(/!=/g,'≠');
 const fallback=()=> /^[A-Za-z]{2,}$/.test(source)?normalizeAnswer(value):'notation:'+source;
 if(source.length>500)return fallback();
 const functions=['arcsin','arccos','arctan','sin','cos','tan','sec','csc','cot','log','ln','exp','abs','root','norm','vec','overline','f','g','C','P'];
 const tokens=source.match(/arcsin|arccos|arctan|overline|sqrt|root|norm|sin|cos|tan|sec|csc|cot|log|ln|exp|abs|vec|pi|(?:\d+(?:\.\d+)?|\.\d+)|[A-Za-zα-ωΑ-Ωπ∞±(),+\-*/^√]/g)||[];
 if(!tokens.length||tokens.join('')!==source)return fallback();
 let pos=0;const peek=()=>tokens[pos];
 function atom(){const t=tokens[pos++];if(t==='('){const a=sum();if(tokens[pos++]!==')')throw Error();return a}if(t==='sqrt'||t==='√')return ['sqrt',atom()];if(functions.includes(t)&&peek()==='('){pos++;const args=[sum()];while(peek()===','){pos++;args.push(sum())}if(tokens[pos++]!==')')throw Error();return ['call',t,args]}if(t==='pi'||t==='π')return ['symbol','π'];if(t&&/^[A-Za-zα-ωΑ-Ω∞]$/.test(t))return ['symbol',t];if(t&&/^(?:\d|\.\d)/.test(t)){let n=t.replace(/^0+(?=\d)/,'');if(n.includes('.'))n=n.replace(/0+$/,'').replace(/\.$/,'');return ['number',n]}throw Error()}
 function power(){let a=atom();if(peek()==='^'){pos++;a=['^',a,unary()]}return a}
 function unary(){if(peek()==='±'){pos++;return ['plusminus',unary()]}if(peek()==='+'){pos++;return unary()}if(peek()==='-'){pos++;return ['neg',unary()]}return power()}
 function product(){let a=unary();while(peek()==='*'||peek()==='/'||peek()&&/^(?:\d|[A-Za-zα-ωΑ-Ω∞(√])/.test(peek())){let op=peek();if(op==='*'||op==='/')pos++;else op='*';a=[op,a,unary()]}return a}
 function sum(){let a=product();while(peek()==='+'||peek()==='-'||peek()==='±'){const op=tokens[pos++];a=[op,a,product()]}return a}
 try{const tree=sum();return pos===tokens.length?(exactNumber(tree)||'math:'+JSON.stringify(tree)):fallback()}catch{return fallback()}
}
function splitAnswerList(value){
 const text=String(value).normalize('NFC').replace(/[！-～]/g,c=>String.fromCharCode(c.charCodeAt(0)-0xfee0)).trim(),prefix=/^([A-Za-zα-ωΑ-Ω])\s*=\s*/.exec(text),body=prefix?text.slice(prefix[0].length):text;
 const stack=[],parts=[];let from=0;
 for(let i=0;i<body.length;i++){const c=body[i];if('([{'.includes(c))stack.push(c);else if(')]}'.includes(c)){if(stack.pop()!=={')':'(',']':'[','}':'{'}[c])return null}else if(c===','&&!stack.length){parts.push(body.slice(from,i).trim());from=i+1}}
...
async function makeHashes(questions,salt){const hashes=[];for(const q of questions){const variants=answerVariants(q.answer);hashes.push(await Promise.all(variants.map(v=>answerHash(salt,v))))}return hashes}
function validQuestions(body){const qs=Array.isArray(body.questions)?body.questions:[];if(qs.length<1||qs.length>40||qs.some(q=>!q||!String(q.answer||"").trim()||String(q.answer).includes("□")||q.type!=="choice"&&(separatedNumbers(q.answer)||splitAnswerList(q.answer)?.parts.some(v=>!v))))return null;const questions=qs.map(q=>({type:q.type==="choice"?"choice":"short",answer:String(q.answer).slice(0,300).trim()}));for(const q of questions){if(q.type==="choice"){const options=q.answer.normalize("NFKC").split("|").map(x=>x.trim());if(options.some(x=>!["1","2","3","4","5"].includes(x)))return null;q.answer=options.join("|")}}return questions}
...
async function updateAssignment(id,request,env){
 ...
 const count=await regradeProgress(id,env,{salt,hashes_json:JSON.stringify(hashes),answer_keys_json:JSON.stringify(questions),question_count:questions.length});
